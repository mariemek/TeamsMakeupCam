...
struct SavePresetSheet: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            Text("Save Preset").font(.title3).fontWeight(.semibold)
            ...
            HStack(spacing: 12) {
                ...
                Button("Save") {
                    ...
                    let preset = MakeupPreset(name: trimmed, settings: settings)
...
struct PresetsListView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                Text("My Presets").font(.title3).fontWeight(.semibold)
                Spacer()
            ...
            if presetStore.presets.isEmpty {
                VStack(spacing: 12) {
                    ...
                    Text("No presets saved yet.").foregroundColor(.secondary)
                    Text("Customize your makeup and tap \"Save as Preset\".")
                    ...
                    ForEach(presetStore.presets) { preset in
                        HStack(spacing: 12) {
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                Text(preset.name)
                                    .fontWeight(presetStore.activePresetID == preset.id ? .semibold : .regular)
        ...
        .alert("Delete \"\(presetToDelete?.name ?? "")\"?", isPresented: $showDeleteConfirm) {
            Button("Delete", role: .destructive) {
                if let p = presetToDelete { presetStore.delete(preset: p) }
            }
